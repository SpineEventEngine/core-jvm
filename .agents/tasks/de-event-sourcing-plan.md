...
| 2     | `jdbc-storage`, `gcloud-jvm`    | **None** — zero `@Apply`. Recompile + test-fallout only (already smoke-built in Phase C). **Phase E obligation (2026-07-12):** override `StorageFactory.createHistoryStorage(context, HistorySpec)` — allocate by the `(sourceType, name)` pair (e.g., table/kind `MyProject_event_history`); the default funnels into `createRecordStorage`, which these vendors key by `sourceType` alone and would collide the histories with the latest-state storage |

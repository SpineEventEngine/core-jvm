...
| 2     | `jdbc-storage`, `gcloud-jvm`    | **None** — zero `@Apply`. Recompile + test-fallout only (already smoke-built in Phase C). **Phase E obligation (2026-07-12):** override `StorageFactory.createHistoryStorage(context, HistorySpec)` — allocate a distinct table/kind per `(sourceType, itemType)` pair, named at the vendor's discretion (e.g., `MyProject_events`); the default funnels into `createRecordStorage`, which these vendors key by `sourceType` alone and would collide the histories with the latest-state storage |
